...
<template>
    <div class="bg-linear-to-br from-indigo-600 to-purple-700 rounded-xl p-6 text-white shadow-md flex flex-col justify-center">
        <h3 v-if="auth.isAuthenticated && auth.isAdmin" class="text-xl font-semibold text-indigo-200! mb-2">Admin Access</h3>
        <h3 v-else-if="auth.isAuthenticated && auth.isStandard" class="text-xl font-semibold text-indigo-200! mb-2">Standard Access</h3>
        <h3 v-else class="text-xl font-semibold text-indigo-200! mb-2">Guest Access</h3>
        ...
        <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
            ...
            <template v-if="auth.isAuthenticated">
                <div class="text-sm text-indigo-100">
                    <span class="font-semibold">User:</span> {{ auth.user?.name }}
                </div>
